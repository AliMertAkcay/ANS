...
Aus der ersten Eigenschaft folgt, dass bei einem begrenzten Ausgangssignal die Spannungsdifferenz null sein muss. Es wird angenommen, dass aufgrund der zweiten Eigenschaft kein Strom in den Eingang des Operationsverstärkers fließt.
...
Gemäß den idealen Eigenschaften des Operationsverstärkers ist die Annahme zulässig, dass der Eingangsstrom nicht in den Operationsverstärker fließt ($I_N = 0$). Folglich gilt die Gleichung für den Ausgangstrom: @eq-VoutStrom
...
Die Herleitung der Übertragungsfunktion basiert auf @eq-Invue und @eq-SKpaImpedanz, welche die Impedanz der Kapazität im Laplace-Bereich beschreibt.
Die Verwendung von @eq-Invue in @eq-SKpaImpedanz führt zu @eq-Integration.
...
Der Term $\frac{1}{s}$ beschreibt das Integrationsverhalten in der komplexen S-Ebene. Der Term $\frac{-1}{R_1 \cdot s}$ entspricht dem negativen Vorfaktor.
...
Die Schaltung wird, wie in Abbildung … dargestellt, mithilfe der grafischen Benutzeroberfläche von KiCad aufgebaut. Die hierfür benötigten elektrischen Symbole lassen sich über die integrierte Bibliothek einfügen (vgl. Abbildung …). Sollte ein spezifisches Bauteil nicht in der Standardbibliothek enthalten sein, besteht die Möglichkeit, benutzerdefinierte Symbole zu erstellen (siehe Abbildung …).
...
Um die Simulationsergebnisse mit der theoretisch berechneten Übertragungsfunktion sowie den Messergebnissen vergleichen zu können, wurden die in Abbildung … gezeigten Befehle verwendet, um die Rohdaten (RAW-Dateien) der Simulation zu exportieren. Diese Dateien wurden anschließend mithilfe der Python-Bibliothek ltspice eingelesen und weiterverarbeitet, um eine systematische Analyse und Gegenüberstellung der verschiedenen Ergebnisse zu ermöglichen.
