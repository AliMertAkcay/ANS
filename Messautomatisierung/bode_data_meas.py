...
messung = ["BPF_Q1_1uF","LPF_Q1_1uF","HPF_Q1_1uF","BSF_Q1_1uF","LPF_Butterworth"]

Anzahl = len(kondensatoren)*len(güte)*len(messung)

print(f"Gesamtanzahl aller Messungen: {Anzahl}")

for i in enumerate(): # Diese schleife sorgt dafür dass alle gennaten mess 
    ready =  input("Sind sie Bereit für die messung: "+messung[i]+" Antworten sie mit Y/N")
    if ready == "Y":
        for freq in freqs:
            print("freq:",freq)
    ...
    # %% Data storage
    Data_IN1 = 'data/IN1_INT_IN_'+messung[i]  # + str(datetime.now().strftime('%Y-%m-%d_%H_%M'))
    Data_IN2 = 'data/IN2_INT_OUT_'+messung[i]  #+ str(datetime.now().strftime('%Y-%m-%d_%H_%M'))
